...
func (Adapter) Detect(start string) (workspace.Unit, error) {
	...
	return workspace.Unit{}, fmt.Errorf("no supported Rust project manifest found from %s", absStart)
}
...
func (Adapter) Validate(ctx context.Context, unit workspace.Unit) error {
	...
	required := []string{"rust-analyzer", "cargo"}
...
func (Adapter) Index(ctx context.Context, unit workspace.Unit, artifactPath string) (indexer.Result, error) {
	...
	if err != nil {
		return indexer.Result{}, fmt.Errorf("run rust-analyzer scip: %w\n%s", err, strings.TrimSpace(string(output)))
	}

	versionCmd := exec.CommandContext(ctx, "rust-analyzer", "--version")
...
func (Adapter) DeriveEdges(ctx context.Context, req indexer.DeriveRequest) ([]store.EdgeData, error) {
	_ = ctx
	return indexer.DeriveOccurrenceEdges(req, "use_declaration"), nil
}
